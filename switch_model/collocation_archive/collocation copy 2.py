...
def define_components(mod):
    ...
    OFFSHORE_PROJECTS is the set of all GENERATION_PROJECT ids associated with the wave
    and offshore wind projects found in WAVE_OSW_PAIRS.csv
    ...
    min_cap_mw_offshore is a parameter (scalar) that represents the minimum capacity that
    an offshore project must build for Built = 1 to be true.
    ...
    mod.OFFSHORE_SITES = Set(ordered=False, dimen=1, initialize= lambda m: set(s for (s, w, o) in m.OFFSHORE_WO))
    mod.WAVE = Set(ordered=False, dimen=1, initialize= lambda m: set(w for (s, w, o) in m.OFFSHORE_WO))
    mod.OSW = Set(ordered=False, dimen=1, initialize= lambda m: set(o for (s, w, o) in m.OFFSHORE_WO))
    ...
    mod.Enforce_Min_Build_Hybrid = Constraint(
        mod.OFFSHORE_GEN_BLD_YRS,
        rule=lambda m, g, p: (
            m.min_cap_mw_offshore * m.BuiltHybrid[m.wave_osw_site_map[g], p]
