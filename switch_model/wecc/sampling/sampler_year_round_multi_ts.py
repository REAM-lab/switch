...
# Connect this to sampling.yaml in switch model
# Change how all values are calculated relative to number_of_ts specified
# Check timepoints rows section and onward


def sample_year_round_multi_ts(
        method_config,
        period_values,
        db_conn=None,
        **kwargs
    ...
    number_of_ts = method_config["number_of_ts"]
    ...
    for i, period in enumerate(period_values):
        ...
        for ts_num in range(1,number_of_ts+1):
        	...
        	timeseries.append((
            	...
            	period_id,
            	f"{label}-year-round-multi-ts-{ts_num}",  # ts_name
            	hours_per_tp,
            	tp_per_year,
            	first_timepoint_utc,
            	last_timepoint_utc,
