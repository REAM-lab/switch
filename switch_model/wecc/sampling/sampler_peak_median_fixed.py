    ...
    for num, date in enumerate(dates):
        if peak:
            ...
            if number_tps == 6:
                ...
                hours_to_sample = list(range(0, 24, delta_t))  # [0, 4, 8, 12, 16, 20]
                tps = pd.concat([
                    df.loc[[day_start + pd.Timedelta(hours=h)]]
                    for h in hours_to_sample
                    if (day_start + pd.Timedelta(hours=h)) in df.index
    			])
            elif number_tps == 24:
